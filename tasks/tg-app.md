...
## Цель (уточнена 24.08.2026 — концепция зафиксирована, см. `SPECIFICATION.md`)
Единый Fullstack-монорепозиторий с развёртыванием на собственном сервере клиента:
- `(site)` — B2B-лендинг агентства TensorLogix + каталог услуг + личный кабинет клиента;
- `(admin)` — Единый кабинет: контент сайта + карточки/функции Mini App клиента;
- `(tma)` — демо Mini App (первый кейс: «Агентство Интеллектуальных Игр»).
